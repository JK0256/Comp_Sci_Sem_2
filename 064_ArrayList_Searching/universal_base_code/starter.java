...
class starter {
	...
	public static void searchArrInt(ArrayList<Integer> arr, int num){
		...
		for (int loop = 0; loop < arr.size(); loop++) {
			if (arr.get(loop) == num) {
				isPresent = true;
				System.out.println("Yes, index of " + num + " is " + loop);
			}
		}
		if (!(isPresent)){
			System.out.println("No, given num does not exist.");
		}
	}
	public static void searchArrString(ArrayList<String> arr, String word){
		boolean isPresent = false;
		for (int loop = 0; loop < arr.size(); loop++) {
			if (arr.get(loop).equals(word)) {
				isPresent = true;
				System.out.println("Yes, index of " + word + " is " + loop);
			}
		}
		if (!(isPresent)){
			System.out.println("No, given word does not exist.");
		}
	}
